const SUPABASE_URL = "https://xxzljsnsgfjjfrtkrxlp.supabase.co"; // 여기 교체
const SUPABASE_KEY = "sb_publishable_IqmOoKKK7I2EmHlmqLqv2w_RVGbvW6A"; // 여기 교체
...
function renderHistory(data) {
  ...
  if (data.length === 0) {
    el.innerHTML =
      '<div class="hist-empty">No entries yet.<br>Save a calculation from the Calculator tab.</div>';
    return;
  }
      ...
      <div class="hist-item">
        <div class="hist-meta">
          ...
          <div class="hist-right">
            ...
            <button class="btn-del" onclick="deleteEntry(${
              h.id
            })" title="Delete">✕</button>
          </div>
        </div>
        <div class="hist-breakdown">
          ${items
            .map(
              (it) =>
                `<span class="pill">${it.label} × ${
                  it.count
                } = € ${it.sub.toFixed(2)}</span>`
            )
            .join("")}
        </div>
      </div>
...
function showTab(name, btn) {
  ...
  document.getElementById("page-calc").style.display =
    name === "calc" ? "block" : "none";
  document.getElementById("page-hist").style.display =
    name === "hist" ? "block" : "none";
